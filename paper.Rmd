...
There are two ways for answers to converge: when the answers are continuous, they can have a smaller range, or a lower variance; when the answers are categorical (i.e. there are only a few options), more answers can be the same. These two cases have been studied with different paradigms.

In the continuous case, the most relevant evidence comes from the literature on 'advice-taking.' In these experiments, participants are called 'judges' who need to make numerical estimates--sometimes on factual knowledge, e.g. 'What year was the Suez Canal opened first? [@yanivReceivingOtherPeople2004], sometimes on knowledge controlled by the experimenters, e.g. 'How many animals were on the screen you saw briefly?' [@mollemanStrategiesIntegratingDisparate2020]. To help answer these questions, participants are given estimates from others, the 'advisors'.
...
There is clear evidence that, both in the continuous and the categorical case, participants infer that convergent opinions are more likely to be correct. There is tentative evidence that participants also infer that individuals who hold more convergent opinions are more likely to be competent. Would that second inference be justified? More precisely, if we consider individuals who have convergent opinions, by contrast with individuals whose opinions are less convergent, or whose opinion diverges from that of a majority, are we justified in thinking that the individuals with more convergent opinions are more likely to be right not only for this specific question, but that they would also be right for other (related) questions? We first provide a general analytic answer, before presenting simulations for the continuous, and then the categorical case.
...
In the continuous case, we imagine a population of informants. The answers of each informant are drawn from a normally distributed random variable, centered on the correct answer (i.e., informants have no systematic bias). The variance of this distribution represents the individuals’ competence: the larger the variance, the lower the competence. If we observe a sample of this population, the sample mean (i.e. the mean of the answers of all the informants) is then the best estimate of the correct answer, and the sample variance (i.e. mean squared distance between the answers and the sample mean) is the best estimate of the population’s average competence (best is understood here as the least volatile estimator). This means that a population of informants whose answers converge more (lower sample variance) is, on average, more competent (informants tend to answer with a lower variance).

We can extend this argument from populations to individuals: Consider that competence varies within a population, such that each informant's answer is drawn from their own individual distribution--always centered on the correct answer, but with different variances. In this case, the distance between each informant's answer and the sample mean provides the best estimate of that informant's competence (i.e. individuals whose answers are further away from the sample mean tend to be less competent).

In the categorical case, we assume that informants answer a question with several categorical answers. We define the competence of an informant as the probability of choosing the correct answer. Due to the law of large numbers, the relative size of the majority--the share of informants who choose the most chosen answer--is, when the population is large enough, a good approximation of the average competence of the population (e.g. if the average competence is .66, and there are enough informants, then approximately 66% of informants will select the right answer). This relationship still exists, but is more noisy for small populations. This means that, at the population level, the more the answers converge, the more the population is, on average, competent (and the larger the population, the stronger the inference).

To make inferences on individual informants' competence on the basis of whether or not their answers converge with those of the majority in the sample, we must assume some distribution of competence in the population (assuming, as above, that the true average population competence is equal to the observed majority size in the sample). Using Bayes theorem, we can then show (see Electronic Supplementary Materials, ESM, section \@ref(analytical)) that the more informants agree with a focal informant, the more the focal informant can be inferred to be competent (Fig. \@ref(fig:figure-Benoit-2)). Additionally, the more competence varies in the population, the more the degree of convergence is informative of an informant's competence. It makes sense: if competence is roughly uniform in the population, then being part of a minority likely reflects bad luck, rather than incompetence. More generally, the fact that the focal individual is right (their 'accuracy') can be inferred more strongly than their competence, as there is noise in the answer choice (an incompetent individual can always pick the correct answer by chance and vice versa). Figure \@ref(fig:figure-Benoit-2) provides an example of a specific categorical choice, for two different distributions of competence.

(ref:figure-Benoit-2) Average estimated competence and accuracy for an individual, depending on how many participants agree with him, in a categorical choice scenario. Here, we assume that 20 individuals answer a 5-choice question. For values higher than four, we assume that the individual is part of the majority. Below four, we assume that the focus individual is part of a minority, and that the majority size is four. Two situations are represented: one in which there is low variance in the distribution of competence in the population (orange line in the insert), and one in which there is high variance in this distribution (blue line in the insert).
...
In order to better understand the influence of different parameters (e.g. degree of convergence, number of individuals) on the relationship between convergence, accuracy, and competence of informants, we conducted simulations.
...
All experiments were preregistered. Preregistration documents, data and code can be found on Open Science Framework [project page](https://osf.io/6abqy/) (<https://osf.io/6abqy/>). All analyses were conducted in R (version 4.2.2) using R Studio. For most statistical models, we relied on the `lme4` package and its `lmer()` function.
